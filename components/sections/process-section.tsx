...
export function ProcessSection() {
  ...
  return (
    <section id="process" className="relative bg-background py-20 overflow-hidden border-t border-foreground/5">
      <div className="max-w-[1400px] mx-auto px-6 md:px-24 mb-4">
        {/* Section Header */}
        <div className="reveal-on-scroll">
          <h3 className="text-[10px] uppercase tracking-[1em] text-muted-foreground mb-8">06 — Process</h3>
          <h2 className="text-6xl md:text-[8vw] font-bold tracking-tighter leading-[0.85] text-foreground">
            The <span className="serif-italic font-light text-muted-foreground italic">Blueprint.</span>
          </h2>
        </div>
      </div>
          ...
          {phases.map((phase) => (
            <div
              ...
              className={`process-card shrink-0 w-[85vw] md:w-[550px] h-[400px] rounded-[3rem] p-10 md:p-12 flex flex-col justify-between relative group overflow-hidden ${
                phase.variant === "light"
                  ? "bg-background dark:bg-white text-foreground dark:text-black border border-foreground/5 dark:border-none"
                  : phase.variant === "darker"
                    ? "bg-secondary dark:bg-[#111111] border border-foreground/5"
                    : "bg-secondary dark:bg-[#0A0A0A] border border-foreground/5"
              }`}
            >
              {phase.variant !== "light" && (
                <div
              ...
              <div className="relative z-10">
                ...
                <h4
                  className={`text-4xl md:text-5xl font-bold tracking-tighter uppercase leading-none ${
                    phase.variant === "light" ? "text-foreground dark:text-black" : "text-foreground"
                  }`}
                >
                  {phase.title}
                </h4>
              ...
              <div className="relative z-10">
                <p
                  className={`text-base md:text-lg leading-relaxed mb-6 ${
                    phase.variant === "light" ? "text-muted-foreground" : "text-muted-foreground"
                  }`}
                >
                  {phase.description}
                </p>
